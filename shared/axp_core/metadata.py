def ensure_model(con, model_id, dimension, revision='unknown'):
    ...
    if conflicts: raise RuntimeError('Embedding configuration changed; reindex required: '+', '.join(sorted(conflicts)))
    con.executemany('INSERT OR IGNORE INTO metadata(key,value) VALUES (?,?)',wanted.items()); con.commit()
