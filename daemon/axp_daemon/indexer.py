...
def scan(con, root, embedder):
    ...
    for path in discover(root):
        ...
        if old and old["sha256"] == digest:
            con.execute(
                "UPDATE documents SET size_bytes=?,modified_unix_ms=? WHERE id=?", (st.st_size, mtime, old["id"])
            )
            result["unchanged"] += 1
            continue
        try:
            ...
            if old:
                ...
                cur = con.execute(
                    "INSERT INTO documents(source_root,path,path_key,extension,size_bytes,modified_unix_ms,sha256,indexed_unix_ms) VALUES(?,?,?,?,?,?,?,?)",
                    (root, str(path), key, path.suffix.lower(), st.st_size, mtime, digest, now),
                )
                doc_id = cur.lastrowid
                result["new"] += 1
            if old:
                con.execute(
                    "UPDATE documents SET path=?,extension=?,size_bytes=?,modified_unix_ms=?,sha256=?,indexed_unix_ms=? WHERE id=?",
                    (str(path), path.suffix.lower(), st.st_size, mtime, digest, now, doc_id),
                )
            for no, (chunk, vector) in enumerate(zip(chunks, vectors)):
                cur = con.execute(
                    "INSERT INTO chunks(document_id,chunk_no,text,page_no,char_start,char_end) VALUES(?,?,?,?,?,?)",
