...
class Embedder:
    def __init__(self,model_id=FASTEMBED_MODEL,cache_dir=None,local_only=True):
        ...
        self._model=TextEmbedding(**kwargs); self._dimension=len(next(iter(self._model.embed(['dimension probe']))))
    @property
    def dimension(self): return self._dimension
    def embed_documents(self,texts): return [list(x) for x in self._model.embed(texts)]
