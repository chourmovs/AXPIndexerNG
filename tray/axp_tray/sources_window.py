...
from axp_core.sources import SourceError, add_source, disable_source, enable_source, list_sources
...
class SourcesWindow:
    def __init__(self, root, db_path):
        ...
        self.tree = ttk.Treeview(self.window, columns=("enabled", "label", "path", "kind", "status", "files", "chunks", "last"),
                                 show="headings", selectmode="browse")
        headings = (("enabled", "Enabled", 65), ("label", "Label", 130), ("path", "Path", 260),
                    ("kind", "Type", 70), ("status", "Status", 90), ("files", "Files", 75),
    ...
    def refresh(self):
        ...
        for row in rows:
            ...
            self.tree.insert("", "end", iid=str(row["id"]), values=("✓" if row["enabled"] else "○", row["label"],
                             row["path"], row["kind"], row["status"].title(), f"{row['documents']:,}",
                             f"{row['chunks']:,}", last))
